...
def melhor_a(pb_left):
	...
	while i < 1.01:
		...
		coord = [-2,0,4,2]
		result = (coord[0]*pa_up*pb_left) + (coord[1]*pa_down*pb_left) + (coord[2]*pa_up*pb_right) + (coord[3]*pa_down*pb_right)
...
max_val = 100
q1 = np.linspace(0,1,max_val)
q2 = np.linspace(0,1,max_val)
  
ax.set_title('cap 30.5 - Coexistência')
ax.set_xlabel('Linha - Prob Lutar')
ax.set_ylabel('Coluna - Prob Lutar')
...
ax.set_title('cap 30.5 - Coexistência')
ax.set_xlabel('proporção de lutadores')
ax.set_ylabel('retorno')

plt.plot(q1,(-2*q1+4*(1-q1)),'-',color='blue', label='Retorno Lutadores',linewidth=2)
plt.plot(q1, (2*(1-q1)),'-',color='red', label='Retorno Cooperadores',linewidth=2)
