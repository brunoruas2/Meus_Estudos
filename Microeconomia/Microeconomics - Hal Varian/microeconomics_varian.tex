...
\\
\\
Eu mudei um pouco o formato da fronteira de possibilidade de produção, mas o importante é que você veja no gráfico abaixo que existe um ponto onde as curvas de indiferença de ambos os agentes são tangentes entre si e, simultaneamente, possuem a mesma inclinação que a curva de posibilidade de produção. Mas atente para o seguinte 
...
Resolvendo para a taxa marginal de substituição
...
Onde C é a produção de cocos, F é a produção de peixes, $L_c$ é a quantidade de trabalho de Robinson e $L_F$ a de Sexta-Feira, $w_C$ é o salário de Rbinson e $w_F$ é o de Sexta-Feira e, por fim, $p_C$ e $p_F$ são os preços de mercado. A quantidade total produzida está restrita à fronteira de possibilidade de produção (que nada mais é do que o reflexo das restrições tecnológicas).
\\
\\
Nosso objetito é maximizar o lucro da empresa, que agora se chama Naufrágios S.A., que será dado em algum ponto no conjunto de produção. Podemos definir que, no ponto de maximização, a empresa contratará uma quantidade ótima de trabalho $L_*$ de modo que os custos de trabalho podem ser descritos como $L^* = w_C L^*_C + w_F L^*_F$. Podemos reescrever nossa função objeto como:

$$ \pi = p_C C + p_F F - L^* $$

Podemos rearranjar a equação para encontrar a solução da quantidade de Cocos

$$ C = \frac{\pi + L^*}{p_C} - \frac{p_F F}{p_C} $$

Essa equação descreve as \textbf{retas de isolucro} da Naufrágios S.A., ou seja, dada uma quantidade ótima de trabalho e lucro, o lucro da empresa varia com a 
