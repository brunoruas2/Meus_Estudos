...
Não tem muito o que falar aqui. Espero que você saiba o que é um gráfico.

\section{Propriedades de funções}

Uma função pode ter algumas características que facilitam a sua descrição. 
\\
\\
...
Uma das implicações de quando uma função é monotônica é que, para cada $x$, eu sempre terei apenas um único $y$ associado. 
\\
\\
Uma \textbf{função inversa} é a função que, sempre que colocarmos um $y$ teremos como resultado o $x$ de alguma função anterior.
